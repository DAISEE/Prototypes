...
Get the adress of another node and add it in `config.toml` in **[network]** part:
```
...
reseal_on_txs = "none"
```
> engine_signer is the account adress used for authorities.
...
Add the account adress in `demo-spec.json` in `"accounts"` part like this:
```
